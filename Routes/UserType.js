...
const UserTypeSchema = require("../model/UserType");
const CountrySchema = require("../model/Country");
const authorize = require("../middlewares/auth");
const { check, validationResult } = require('express-validator');
// User Type
router.post("/save_usertype",
    [
        check('user_type')
            .not()
            .isEmpty()
    ...
    (req, res, next) => {
       ...
       console.log(req.body);
        ...
        if (!errors.isEmpty()) {
            ...
            const usertypes = new UserTypeSchema({
                    user_type: req.body.user_type,
                    is_active: 'Y',
                    created:new Date()
            });
            usertypes.save().then((response) => {
                res.status(201).json({
...
router.route('/EditUserType/:id').get(authorize, (req, res, next) => {
...
router.get("/usertypelist", async (req,res)=>{
  var response={status:false,msg:"",data:""};
 
  try {
      const results = await UserTypeSchema.find({});
